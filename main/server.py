...
def sigint_handler(signum, frame):
    print '[info] User interrupt ! shutting down\n'
...
def chat_server():
    ...
    print "[info] Server started on port " + str(PORT) + "\n"
    ...
    while 1:
        ...
        for sock in ready_to_read:
            ...
            if sock == server_socket:
                ...
                print "[info] user (%s, %s) connected\n" % addr

                broadcast(server_socket, sockfd, encrypt(key,"[%s:%s] entered our chatting room\n" % addr))

            else:
                try:
                    datap = sock.recv(RECV_BUFFER)
                    data = decrypt(key,datap)
                    if data:
